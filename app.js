...
app.use('/places/search', function (req, res, next) {
       ...
       var ac = req.query.a;
        ...
        if (ll) {
          ...
          client.query("select array_to_json(array_agg(row_to_json(t))) as places from (select id, name,(select array_to_json(array_agg(row_to_json(ps))) from (select service_name as name, service_short_description as description, service_id, name as type, identifier from place_services inner join service_types on service_types.id = place_services.service_type_id where place_id = places.id) ps) as services from places where ST_CONTAINS(coordinates, ST_GeomFromText('POINT(" + longitude  + " " +  latitude + ")'))) t", null, function (err, results) {
